...
class Traffic_Light:
    ...
    def  TL_Logic(self, color, coordinates):
        ...
        if y1 < 0.6 and y1 > 0.2 and x1 > 0.5: #relative positions for now
            if color == "green":
                if False:#self.RT.is_moving(): ###this needs to be changed to a function that can get the velocity of the car
                    pass
                else:
                    self._TSpeed =  100#self.RT.get_velocity()
                #if car is not moving, find last speed limit and tell car to go at that speed limit

            elif color == "red":
                if False:#not self.RT.is_moving():
                    pass
                elif True: #self.RT.is_moving() and y1<0.4: #arbitrary value
                    self._TSpeed = 69
            else:
